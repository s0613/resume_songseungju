 ...
 * 제외된 문서는 항상 목록으로 출력한다 — 조용히 빠지는 일은 없어야 한다.
 */
...
/** 이 중 하나라도 본문에 걸리면 공개하지 않는다. */
const BLOCKERS = [
    { name: "고객사·타사명", re: /인천항만|POSCO|포스코|samchungdang|삼청당|채널톡/i },
    { name: "로컬 절대경로", re: /\/Users\/|\/home\/[a-z]/ },
    ...
    { name: "자격증명", re: /sk-[A-Za-z0-9]{15,}|AKIA[0-9A-Z]{14}|ghp_[A-Za-z0-9]{20,}|xox[baprs]-|-----BEGIN/ },
]
...
if (blocked.length) {
    console.log("\n제외된 문서 (본문에 아래 항목이 있어 공개하지 않음):")
    for (const { rel, reason } of blocked) console.log(`  [${reason}] ${rel}`)
}
